# WarrantyWise User & Developer Guide

Welcome to WarrantyWise!

Please see the comprehensive ELI5 Tanglish Guide here:
👉 **[GUIDE_TANGLISH.md](GUIDE_TANGLISH.md)**

### Quick Start:
- Double-click **`start.bat`** to start the application and launch your browser.
- Double-click **`stop.bat`** to stop the application.

## Demo / Presentation Data Guide

When presenting the project, use the following dummy data to populate the fields accurately and make the dashboard look comprehensive.

### 1. Register & Login
- **Name:** Hariharan P
- **Email:** hari@demo.com
- **Password:** Hari1234

### 2. Add a Product (e.g., Laptop)
- **Product Name:** MacBook Pro M3
- **Category:** Electronics
- **Brand:** Apple
- **Model Name:** M3 Max
- **Model Number:** A2991
- **Serial Number:** C02G123456
- **Color:** Space Black
- **IMEI Number:** (Leave Blank or N/A)
- **Purchase Date:** 01-Jan-2024
- **Purchase Price:** 150000
- **Purchase Mode:** ONLINE
- **Store / Retailer:** Apple Store, Chennai
- **Product Status:** IN_USE
- **Condition:** NEW
- **Storage Location:** Home Office Desk
- **Notes:** Bought for Final Year Project

### 3. Add Warranty
- **Product:** MacBook Pro M3
- **Warranty Type:** Manufacturer
- **Provider Name:** Apple Care+
- **Policy Number:** POL-9876543
- **Start Date:** 01-Jan-2024
- **End Date:** 31-Dec-2026
- **Policy Status:** ACTIVE
- **Coverage Details:** Covers accidental damage, battery replacement, and hardware failures.
- **Terms & Conditions:** Subject to standard Apple Care+ terms. Liquid damage may incur a service fee.
### 4. Add Service Record
- **Product:** MacBook Pro M3
- **Service Date:** 15-May-2024
- **Service Center:** iCare Authorized
- **Issue:** Keyboard keys not working
- **Cost:** 0
- **Status:** Completed
